...
public class BHandler {
    ...
    public static CHandler handler = new CHandler();
    ...
    public static void handleMsg(Message msg) {
        ...
        if (!msg.getAttachments().get(0).getFileName().endsWith(".msch") && !msg.getAttachments().get(0).getFileName().endsWith(".msav") && !msg.getAttachments().get(0).getFileName().endsWith(".zip")) {
            System.out.println(msg.getAttachments().get(0).getFileName());
            msg.delete();
            return;
        }
        ...
        if (msg.getAttachments().get(0).getFileName().endsWith(".msch")) {
            schema(msg, a);
        }
        if (msg.getAttachments().get(0).getFileName().endsWith(".zip")) {
            mod(msg, a);
        }
    ...
    public static void schema(Message msg, Message.Attachment a) {
        try {
            Schematic schem = handler.parseSchematicURL(a.getUrl());
            System.out.print("***");
            BufferedImage preview = handler.previewSchematic(schem);
    ...
    public static void art(Message msg, Message.Attachment a) {
        ...
        try {
            Streams.copy(CHandler.download(a.getUrl()), new FileOutputStream(artFile));
        } catch (Exception e) {
            return;
        }
    ...
    public static void mod(Message msg, Message.Attachment a) {
        ...
        try {
            Streams.copy(CHandler.download(a.getUrl()), new FileOutputStream(modFile));
        } catch (Exception e) {
            return;
        }
    ...
    public static void karta(Message msg, Message.Attachment a) {
        try {
            Map map = handler.readMap(CHandler.download(a.getUrl()));
            new File("maps/").mkdir();
